...
test('test1', async ({ page }) => {
  ...
  await page.locator('(//table[@id="productTable"]//td//input)[4]').uncheck();
  ...
  for (let i = 0; i < count; i++) {                        //knjm doubt again again practice pannu 
    await (checkboxes.nth(i)).check()
    await expect(checkboxes.nth(i)).toBeChecked()//tobechecked 1 locator ku thn check pannum athunala thn nth(i) la pottrukom 
  }
  await page.waitForTimeout(3000)
