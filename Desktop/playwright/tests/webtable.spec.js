...
test('webtable', async ({ page }) => {
    ...
    // const filtr = await rows.filter({
    //     has: page.locator("td"),
    //     hasText : "Manchester"

    // }) 
    // await filtr.locator('input').check();
    // expect(filtr.locator('input')).toBeChecked()
    // await page.waitForTimeout(3000)

    await tables(rows, page, "Sarah Lee");
    await tables(rows, page, "Anand Jeff")
    

})

async function tables(rows, page, name) {
    const filtr = await rows.filter({
        has: page.locator("td"),
        hasText: name
